<x-app-layout>
    <div class="mx-auto w-full space-y-5 sm:space-y-6" x-data="{
        ...
        submitDelete() {
            ...
            if (!confirm('Permanently delete ' + this.selected.length + ' feeder survey(s)? Linked DTR / consumer surveys on this feeder path will also be removed. This cannot be undone.')) return;
        ...
        <section class="al-hero relative p-6 sm:p-7">
            <div class="relative z-10">
                ...
                <h1 class="al-display mt-3 text-3xl font-bold text-white sm:text-4xl">Feeder Surveys</h1>
        ...
        <section class="al-panel p-5 sm:p-6">
            <form method="GET" action="{{ route('feeder-surveys.index') }}" class="grid gap-3 md:grid-cols-3 xl:grid-cols-4">
                <div>
                ...
                <div>
                    ...
                    <select name="status" class="al-input">
                        ...
                        <option value="draft" @selected($filters['status'] === 'draft')>DTR Pending</option>
                        <option value="sld_pending" @selected($filters['status'] === 'sld_pending')>SLD Pending</option>
        ...
        @if($viewed)
            <section class="al-panel overflow-hidden">
                ...
                <form x-ref="bulkForm" method="POST" action="{{ route('feeder-surveys.bulk-delete') }}" class="hidden">
                    @csrf
                </form>
                    ...
                    <table class="min-w-full text-left text-xs">
                        <thead class="bg-seas-950 text-white">
                            <tr>
                                ...
                                <th class="px-3 py-2">DTR Progress</th>
                        ...
                        <tbody class="divide-y" style="border-color: var(--al-border)">
                            @forelse($surveys as $row)
                                <tr class="hover:bg-black/[0.02]">
                                    ...
                                    <td class="px-3 py-2">
                                        <div class="font-semibold text-theme">{{ $row->feeder_name ?: '—' }}</div>
                                        <div class="text-muted">{{ $row->feeder_code }} · {{ $row->substation_name }}</div>
                                    </td>
                                    ...
                                    <td class="px-3 py-2">
                                        <span class="rounded-full px-2 py-0.5 text-[10px] font-bold uppercase
                                            {{ in_array($row->status, ['approved', 'completed'], true) ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300' : ($row->status === 'rejected' ? 'bg-rose-100 text-rose-800 dark:bg-rose-950/50 dark:text-rose-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-950/50 dark:text-amber-300') }}">
                                            {{ $row->display_status }}
                                        </span>
                                    </td>
                                    <td class="px-3 py-2 whitespace-nowrap text-muted">{{ ($row->surveyed_at ?? $row->created_at)?->format('d M Y, H:i') ?? '—' }}</td>
                                    <td class="px-3 py-2 font-semibold text-theme">{{ $row->dtrs_completed }} / {{ $row->dtrs_expected }}</td>
                                    ...
                                    <td class="px-3 py-2 text-right whitespace-nowrap">
                                        <a href="{{ route('feeder-surveys.show', $row) }}" class="font-bold text-volt hover:underline">Open →</a>
                                        <form method="POST" action="{{ route('feeder-surveys.bulk-delete') }}" class="inline ml-2"
                                              onsubmit="return confirm('Permanently delete this feeder survey? Linked DTR / consumer surveys on this feeder path will also be removed.');">
                                            @csrf
                            ...
                            @empty
                                <tr>
                                    <td colspan="11" class="px-5 py-10 text-center text-muted">No feeder surveys match these filters.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
